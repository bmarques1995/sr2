...
def main():
    ...
    package_output_dir = "brotli"
    commit_hash_release = "ed738e842d2fbdf2d6459e39267a633c4a9b2f5d"
    brotli_dir = append_paths(module_destination, "modules", package_output_dir)
    build_dir = append_paths(module_destination, "dependencies", os_name, package_output_dir)
    brotli_cmake_dir = brotli_dir

    clone_and_checkout("https://github.com/google/brotli.git", destination=brotli_dir, commit_hash=commit_hash_release)

    if os_name == "windows":
        run(f'cmake -S "{brotli_cmake_dir}" -B "{build_dir}" '
